...
const SalesScreen = () => {
  ...
  const { data: sales, isLoading: isSalesLoading } = useQuery({
    queryKey: ["/api/user/[id]/sales"],
    queryFn: () => getUserSales(userId as string),
  });

  const Sale: React.FC<OrderType> = (sale) => {
    return (
      <Pressable
        className="pb-3"
        onPress={() =>
      ...
      >
        <View className="w-full space-y-2 rounded-2xl bg-neutral-50 p-2 shadow-lg shadow-neutral-300">
          ...
          <Text className="font-general-sans-regular pl-3">{sale.date}</Text>
          ...
          <View className="w-full flex-row items-center justify-between border-t-[1px] border-t-neutral-200 px-3 py-1">
            <Pressable
              className="p-1"
              onPress={() => console.log("track sale")}
            >
          ...
          <View className="h-full w-full items-center">
            <View className="aspect-square w-[90%] items-center">
              ...
              <Text className="font-general-sans-medium w-full self-center pt-4 text-center text-sm text-sky-900">
                Come back later after you sell some listings.
              </Text>
            </View>
          </View>
        </View>
