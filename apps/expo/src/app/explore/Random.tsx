...
const RandomLisitngs: React.FC = () => {
  const { getRandomListings } = useAPI();
  ...
  const {
    ...
    queryKey: ["listings"],
    queryFn: () => getRandomListings(userId as string, undefined),
  });
  ...
  const onRefresh = async () => {
    ...
    recommendations?.listings.sort(() => Math.random() - 0.5);
    setRefreshing(false);
  };
