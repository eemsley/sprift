...
export default async function handler(
  ...
  try {
    ...
    const savedExists = !!(await prisma.savedForLater.findFirst({
    ...
    if (savedExists) {
      return res.status(409).json({
        ...
        code: 409,
        message: "Conflict",
        details: "The user has already added this listing to their saved for later.",
      });
    }

    // Create new like listing entry
    const saveListing = await prisma.savedForLater.create({
      data: {
        user: {
    ...
    return res.status(201).json(saveListing);
  } catch (err) {
