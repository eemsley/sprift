...
export default async function handler(
  ...
  try {
    ...
    const dislikedListing = await prisma.dislike.findFirst({
      where: {
        userId: user.id,
        listingId,
      },
    ...
    if (!dislikedListing) {
      return res.status(404).json({
        ...
        code: 404,
        message: "Not Found",
        details: "You have not disliked the provided listing.",
      });
    }

    // Remove listing from dislikes
    const removeDislike = await prisma.dislike.delete({
      where: {
        userId_listingId: {
          userId: user.id,
    ...
    return res.status(202).json(removeDislike);
  } catch (err) {
