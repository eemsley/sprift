...
export default async function handler(
  ...
  try {
    ...
    const likeExists = !!(await prisma.dislike.findFirst({
    ...
    if (likeExists) {
      return res.status(409).json({
        ...
        code: 409,
        message: "Conflict",
        details: "The user has already disliked this listing.",
      });
    }

    // Create new dislike listing entry
    const likeListing = await prisma.dislike.create({
      data: {
        user: {
